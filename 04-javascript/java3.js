...
// ### var
// - 선언 단계와 할당 단계가 동시에 진행이 되며, 초기화에는 undefined 값이 들어간다.
...
// - 초반에 선언을 할 때 반드시 **초기화**를 동시에 진행해야된다.
...
// var a2 = 5;  // var 키워드 중복 선언 가능
console.log(a2);

///////////////////////////////////////////////////////////////////////////////////////
// ** var  변수의 문제점 **
// 1. 중간에 같은 이름의 변수를 다시 선언해도 기존의 변수에 덮어 씌움.
//  - 변수를 선언 했다는 것은 분명히 다른 데이터를 넣으려는 것인데 기존의 데이터에 덮어 씌우면?!
//    -> 문제 발생
// 2. 변수가 {블록 단위} 에서 끝나는 것이 아니라, 자기 마음대로 전역으로 돌아다니면서 영향력을 행사함.
//    -> 의도치 않은 문제 발생.
// 3. 호이스팅 문제
...
// 변수 x가 선언되기 전에 사용하였지만, JS 엔진은 선언을 최상단으로 올려 undefined를 출력한 후, 나중에 변수를 초기화하여 값을 할당
...
// A) camelCase => ex) redApple, isWater
// A) snake_case => ex) red_apple, is_water

// Q) 상수란?
// A) 프로그램이 실행되는 동안 값이 변하지 않는 데이터를 나타냄
// - 한 번 정의되면 프로그램 실행 중에는 변경되지 않아야 함. (고정된 값)
// - 보통 대문자로 표기 (개발자 커뮤니케이션 내에서 국룰) => ex) TOMATO, APPLE
