...
// "soket.io" 모듈 불러오기.
// webSoket 기반, 실시간 양방향 데이터 전송 지원 라이브러리.
...
io.on("connection", (socket) => {
    ...
    // });
    // socket.on("bye", (data) => {
    //     socket.emit("byekr", { who: "study", msg: "바이~~" });
    // });
    ...
    // - io.emit(event_name, data) : 서버에 접속된 모든(all) 클라이언트에게 전송
    // - io.to(소캣아이디).emit(event_name, data) : 소켓아이디에 해당하는 클라이언트

    socket.on("setNick", (nick) => {
        console.log(`닉네임 설정 완료 :: ${nick}님 입장`);

        // [실습3-2-1]
        // 프론트에서 입력한 nick이 중복되는지 확인 필요 => nick이 nickObjs에 존재하는지 검사
