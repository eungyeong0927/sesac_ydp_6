...
app.use(
    session({
        ...
        cookie: {
            ...
            secure: false, // http에서 사용 가능하도록 (true라면 https에서만 사용 가능)
            maxAge: 7 * 1000, // 단위 (ms)
            // expires: 만료기간 설정
        },
    })
...
app.get("/set", (req, res) => {
    ...
    // // console.log(req.query) // {}, {name: x, age: y} => 모두 true가 되기 때문에 if문 사용
    if (req.query.name) {
        // GET /set?name=a&age=1
