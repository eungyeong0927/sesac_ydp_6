...
-- left outer join 
select * from instructor I left outer join teaches T on I.id = T.id; -- 왼쪽에 대한 모든 것을 join하기 때문에 null값이 같이 나옴

-- right outer join
select * from instructor I right outer join teaches T on I.id = T.id; -- 오른쪽에 대한 모든 것을 join하기 때문에 null값이 같이 나옴

-- full outer join
-- mysql 에서는  full outer join을 지원하지 않음
-- union 이라는 키워드를 통해 full outer join을 구현할 수는 있음
