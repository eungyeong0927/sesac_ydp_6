...
select * from customer where custname like '_지'; -- 두번째 글자가 '지'인 이름이 2글자인 사람 / 검색 결과 :0
...
-- 2000년생 이후 출생자 중에서 주소를 기준으로 내림차순 그리고 아이디를 기준으로 내림차순 검색하는데 최초의 4개만 추출
...
select sum(amount) as 'total_amoout' from orders;
...
select count(distinct custid) from orders; -- 회원 중 구매전환이 이루어진 고객 수

-- ---------------------------------------------------------------

-- 9. group by : "~별로"
-- select 절에서, group by 에서 사용한 사용한 속성과 집계함수만 사용 가능
